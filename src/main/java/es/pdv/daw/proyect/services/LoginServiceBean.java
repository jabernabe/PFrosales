...
/**
 * Clase que gestiona los servicios de acceso a datos referentes a la gestion de usuarios.
 * @author jabd
 *
 */
@Service
public class LoginServiceBean implements LoginService {
	...
	/**
	 * Propiedad que encapsula el objeto de envio de notificaciones mediante email.
	 */
	@Autowired
	...
	public List<Usuarios> findAllUsers() {
		List<Usuarios> usuarios = repository.findAllByOrderBySurnameAsc();
		return usuarios;
	}
	...
	public UserValidate updateUser(Usuarios usuario) {
		...
		if (num>0){
			...
			if (usuario.getIdUsuario()== user.getIdUsuario()){
				...
				userValidate.setUserExist(true);
				userValidate.setUsuario(usuario);
				logger.info("Han sido actualizados correctamente los datos del usuario con login: "+usuario.getLogin());
			}
			else{
				userValidate.setMessage("Error. Login "+usuario.getLogin()+" no disponible.");
				userValidate.setUserExist(false);
				userValidate.setUsuario(usuario);
				logger.info("Intento fallido de actualizacion de datos. El login "+usuario.getLogin()+" ya existe.");
			}
	...
	public UserValidate deleteUser(Usuarios usuario) {
		...
		if (num<1){	 
			...
			userValidate.setUserExist(true);
			userValidate.setUsuario(usuario);
			logger.info("El usuario con login "+usuario.getLogin()+" ha sido eliminado del sistema correctamente.");
		}
